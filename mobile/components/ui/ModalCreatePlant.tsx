...
export default function ModalCreatePlant({
  visible,
  onClose,
}: ModalCreatePlantProps) {
  const { createPlant, plantSchema } = usePlants();

  const today = new Date();
  const todayBR = `${String(today.getDate()).padStart(2, "0")}/${String(
    today.getMonth() + 1
  ...
  const handleCreate = async () => {
    try {
      ...
      if (err?.issues) {
        ...
        console.log("Erro ao criar planta:", err);
      }
    } finally {
      setLoading(false);
    }
